...
## 为什么 fork 它，而不是自己写一次性脚本？

当你需要的是一个可复用的抓取基础设施，而不是一堆会逐渐分叉、难以维护的渠道脚本时，Retail Scrapers 更合适。

| 一次性脚本 | Retail Scrapers |
|---|---|
| 每个脚本都有隐藏假设 | 统一记录、校验、运行参数和 CLI 行为 |
| 每个测试都访问真实网站 | 优先离线 fixture 测试；live smoke 手动触发 |
| 很难区分半失败和成功 | 严格模式会在输出不完整时失败 |
| 数据存储和提取逻辑混在一起 | 包只负责提取；存储和匹配由你的流水线决定 |
| 新渠道从空文件开始 | `retail-scrape scaffold --with-fixtures` 直接生成 adapter 路径 |

适合：

- 需要从多个公开零售网站抓 catalog 或价格数据的 fork。
- 正在自建价格历史、告警、商品匹配或分析层的团队。
- 想复用真实 ecommerce 页面 adapter 模式的开发者。

不适合：

- 抓取私有、登录后或个人数据。
- 未确认目标网站使用边界的高频抓取。
- 需要开箱即用的 dashboard 或数据库产品。
